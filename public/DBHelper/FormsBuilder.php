  ...
  function __construct($controllersPath, $viewsPath, $classesPath, $tablesList, $replaceRouteForms, $cssClass)
  {
    ...
    while ($o=$sqlObj->FetchObject())
    {
      ...
      if ($replaceRouteForms && !$failedRouterFile) {
        ...
        fwrite($routerFile, "\n");
        fwrite($routerFile, "\techo \$blade->run(\"".$db->GetTableName().".list\", ['controller' => '".$db->GetTableName()."', '".$db->GetTableName()."' => \$obj ]);");
        fwrite($routerFile, "\n");
        fwrite($routerFile, "});");
        fwrite($routerFile, "\n");
        ...
        fwrite($routerFile, "\tglobal \$blade;");
        fwrite($routerFile, "\n");
        fwrite($routerFile, "\techo \$blade->run(\"".$db->GetTableName().".edit\", ['controller' => '".$db->GetTableName()."', '_".$db->GetTableName()."' => \$obj]);");
        fwrite($routerFile, "\n");
        fwrite($routerFile, "});");
        fwrite($routerFile, "\n");
        ...
        fwrite($routerFile, "\tglobal \$blade;");
        fwrite($routerFile, "\n");
        fwrite($routerFile, "\techo \$blade->run(\"".$db->GetTableName().".edit\", ['controller' => '".$db->GetTableName()."', '_".$db->GetTableName()."' => new \\LazyMePHP\\Classes\\".$db->GetTableName()."()]);");
        fwrite($routerFile, "\n");
        fwrite($routerFile, "});");
        fwrite($routerFile, "\n");
        ...
        foreach ($db->GetTableFields() as $field) {
          if (!$field->IsAutoIncrement() || !$field->IsPrimaryKey())
          {
            ...
            if ($field->HasForeignKey() && !is_null($field->GetForeignField()))
            {
              ...
              fwrite($viewFile, "\n");
              fwrite($viewFile, "\t\t@foreach(\$".$field->GetForeignTable()." as \$v)");
              fwrite($viewFile, "\n");
