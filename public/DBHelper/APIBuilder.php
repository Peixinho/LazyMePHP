    ...
    if (!is_dir(APP::ROOT_PATH()."/".$apiPath)) \LazyMePHP\Helper\MKDIR(APP::ROOT_PATH()."/".$apiPath);
		...
		if ($replaceRouteApi) {
          ...
          fwrite($routerFile, "use Pecee\SimpleRouter\SimpleRouter;");
            ...
            fwrite($routerFile, "\n");
            fwrite($routerFile, "\t\$obj = ".$o->Table."::Default();");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "\techo json_encode(\$obj->Serialize(true, \$GLOBALS['API_FIELDS_AVAILABLE']));");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "});");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "SimpleRouter::get('/api/".$o->Table."/{".$db->GetPrimaryFieldName()."}', function (\$".$db->GetPrimaryFieldName().") {");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "\t\$obj = ".$o->Table."::Edit(\$".$db->GetPrimaryFieldName().");");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "\techo json_encode(\$obj->Serialize(true, \$GLOBALS['API_FIELDS_AVAILABLE']));");
            fwrite($routerFile, "\n");
            fwrite($routerFile, "});");
