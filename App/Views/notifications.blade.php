...
.notification {
    position: relative;
    padding: 16px 20px;
    padding-right: 50px; /* Make room for close button only */
    padding-top: 40px; /* Make room for badge at top */
    margin: 8px 0;
    border-radius: 12px;
...
/* Dark mode support - but preserve our beautiful gradients */
@media (prefers-color-scheme: dark) {
    /* Only apply dark mode to elements that don't have specific styling */
    .notification:not(.notification-success):not(.notification-error):not(.notification-warning):not(.notification-info):not(.notification-debug):not(.notification-critical) {
        background: rgba(30, 30, 30, 0.95);
        border-color: rgba(255,255,255,0.1);
        color: #ffffff;
    }
    
    /* Preserve category badge colors in dark mode */
    .notification[data-category="system"] .notification-category {
        background: linear-gradient(135deg, rgba(220, 53, 69, 0.25) 0%, rgba(220, 53, 69, 0.15) 100%);
        color: #dc3545;
        border-color: rgba(220, 53, 69, 0.3);
...
<script>
// Advanced Notification System JavaScript
(function() {
    'use strict';
    
    // Initialize notification system when DOM is ready
    document.addEventListener('DOMContentLoaded', function() {
        // Get notifications from NotificationHelper
        <?php 
