...
- **`App/Api/TestUsers.php`, `TestProducts.php`, `sqlite_sequence.php`** and `App/ApiFieldMask.php` are legacy artifacts from the old per-table REST generator. `ApiFieldMask.php` is tracked/tested; the `App/Api/*.php` files are **not tracked in git** (local-only, kept alive incidentally by one unit test) — don't treat them as the current pattern for building REST endpoints.
